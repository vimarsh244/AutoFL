...
def load_datasets(partition_id: int):
    ...
    if fds is None:
        ...
        fds = FederatedDataset(dataset="kitti", partitioners={"train": partitioner})
    partition = fds.load_partition(partition_id)

    # Divide data on each node: 80% train, 20% test
    partition_train_test = partition.train_test_split(test_size=0.2, seed=42)

    # Define domains based on weather and road type
    domains = {
        "sunny_urban": {"weather": "sunny", "road_type": "urban"},
        "sunny_highway": {"weather": "sunny", "road_type": "highway"},
        "sunny_residential": {"weather": "sunny", "road_type": "residential"},
        "cloudy_urban": {"weather": "cloudy", "road_type": "urban"},
        "cloudy_highway": {"weather": "cloudy", "road_type": "highway"},
        "cloudy_residential": {"weather": "cloudy", "road_type": "residential"},
        "rainy_urban": {"weather": "rainy", "road_type": "urban"},
        "rainy_highway": {"weather": "rainy", "road_type": "highway"},
        "rainy_residential": {"weather": "rainy", "road_type": "residential"},
    }
    ...
    for domain_name, attributes in domains.items():
        ...
        weather_indices = filter_by_attributes(
            partition_train_test["train"], "weather", attributes["weather"]
        )
        road_indices = filter_by_attributes(
            partition_train_test["train"], "road_type", attributes["road_type"]
        )
        domain_indices = list(set(weather_indices) & set(road_indices))
    ...
    for domain_name, attributes in domains.items():
        ...
        weather_indices = filter_by_attributes(
            testset, "weather", attributes["weather"]
        )
        road_indices = filter_by_attributes(
            testset, "road_type", attributes["road_type"]
        )
        domain_indices = list(set(weather_indices) & set(road_indices))
    ...
    benchmark = benchmark_from_datasets(
        train_datasets=train_experiences, test_datasets=test_experiences
    )
...
def get_dataloaders(partition_id: int):
    ...
    for exp in benchmark.test_stream:
        test_loaders.append(DataLoader(exp.dataset, batch_size=BATCH_SIZE))
