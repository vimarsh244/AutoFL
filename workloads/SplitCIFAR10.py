...
def load_datasets(partition_id: int):
    ...
    for task_id, task_classes in enumerate(class_splits):
        ...
        print(
            f"train task {task_id}: classes {task_classes} -> {list(class_mapping.values())} ({len(task_dataset)} samples)"
        )
    ...
    for task_id, task_classes in enumerate(class_splits):
        ...
        print(
            f"test task {task_id}: classes {task_classes} -> {list(class_mapping.values())} ({len(task_test)} samples)"
        )
