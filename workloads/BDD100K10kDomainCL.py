...
class BDD100K10kDataset(Dataset):
    ...
    def _load_data(self):
        ...
        if not self.images_dir.exists():
            print(
                f"Warning: Images directory {self.images_dir} not found. Using fallback mock data."
            )
        ...
        if not self.labels_dir.exists():
            print(
                f"Warning: Labels directory {self.labels_dir} not found. Using fallback mock data."
            )
            self._load_mock_data()
            return
        ...
        for img_path in image_files:
            ...
            try:
                ...
                metadata = {
                    ...
                    "num_objects": len(
                        label_data.get("frames", [{}])[0].get("objects", [])
                    ),
                }
...
def load_datasets(partition_id: int):
    ...
    for name, conditions in domains.items():
        ...
        base_ds = (
            client_train.dataset if isinstance(client_train, Subset) else client_train
        )
    ...
    benchmark = benchmark_from_datasets(
        train_datasets=train_exps, test_datasets=test_exps
    )
    return benchmark
