...
NUM_TASKS = getattr(
    cfg.cl, "num_experiences", 10
)  # Default to 10 tasks for 100 classes
...
def load_datasets(partition_id: int):
    ...
    print(
        f"class splits for {NUM_TASKS} tasks: {[f'classes {s[0]}-{s[-1]}' for s in class_splits]}"
    )
    ...
    for task_id, task_classes in enumerate(class_splits):
        ...
        class_mapping = {
            orig_class: new_class for new_class, orig_class in enumerate(task_classes)
        }
        ...
        print(
            f"train task {task_id}: classes {task_classes[0]}-{task_classes[-1]} -> 0-{len(task_classes)-1} ({len(task_dataset)} samples)"
        )
    ...
    for task_id, task_classes in enumerate(class_splits):
        ...
        print(
            f"test task {task_id}: classes {task_classes[0]}-{task_classes[-1]} -> 0-{len(task_classes)-1} ({len(task_test)} samples)"
        )
