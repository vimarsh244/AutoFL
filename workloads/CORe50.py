...
def load_datasets(partition_id: int):
    ...
    print(
        f"CORe50 benchmark created with {len(train_stream)} train experiences and {len(test_stream)} test experiences"
    )
    ...
    print(
        f"Client {partition_id} assigned {len(client_train_experiences)} train experiences and {len(client_test_experiences)} test experiences"
    )
    ...
    for exp in client_train_experiences:
        train_loader = DataLoader(
            exp.dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=2
        )
        train_loaders.append(train_loader)

    for exp in client_test_experiences:
        test_loader = DataLoader(
            exp.dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=2
        )
    ...
    if client_test_experiences:
        ...
        combined_test_dataset = torch.utils.data.ConcatDataset(
            [exp.dataset for exp in client_test_experiences]
        )
        combined_test_loader = DataLoader(
            combined_test_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=2
        )
    else:
