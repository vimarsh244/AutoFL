...
class KITTIDataset(Dataset):
    ...
    def __init__(
        self, root_dir, split="train", task="object_detection", transform=None
    ):
...
def create_kitti_domain_experiences(
    dataset, domain_conditions, transform, n_experiences=2
):
...
def load_datasets(partition_id: int, data_dir: str = "./data/kitti"):
    ...
    domains = [
        {
            "name": "urban_sunny",
            "conditions": {"road_type": "urban", "weather": "sunny"},
        },
        {
            "name": "highway",
            "conditions": {"road_type": "highway", "weather": ["sunny", "cloudy"]},
        },
        {
            "name": "residential",
            "conditions": {"road_type": "residential", "traffic": ["low", "medium"]},
        },
        {
            "name": "challenging",
            "conditions": {"weather": ["cloudy", "overcast"], "traffic": "high"},
        },
    ]
    ...
    for domain in domains:
        transform = DOMAIN_TRANSFORMS.get(
            domain["name"], DOMAIN_TRANSFORMS["urban_sunny"]
        )
        experiences = create_kitti_domain_experiences(
            client_train, domain["conditions"], transform, n_experiences=2
        )
    ...
    for domain in domains:
        ...
        test_exp = create_kitti_domain_experiences(
            test_dataset, domain["conditions"], transform, n_experiences=1
        )
        if test_exp:
