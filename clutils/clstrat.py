...
def make_cl_strat(net):
    ...
    eval_plugin = EvaluationPlugin(*metrics, loggers=[interactive_logger, text_logger])
    ...
    if strategy_name == "plora":
        ...
        cl_strategy = PLoRAStrategy(
            net, plora_config, num_clients=cfg.server.num_clients
        )
        return cl_strategy, None
    elif strategy_name == "fedcprompt":
        fedcprompt_config = getattr(cfg, "fedcprompt", {})
        cl_strategy = FedCPromptStrategy(
            net,
            fedcprompt_config,
            num_clients=cfg.server.num_clients,
            num_tasks=cfg.cl.num_experiences,
        )
        return cl_strategy, None
    elif strategy_name == "fedet":
        fedet_config = getattr(cfg, "fedet", {})
        cl_strategy = FedETStrategy(
            net,
            fedet_config,
            num_clients=cfg.server.num_clients,
            num_tasks=cfg.cl.num_experiences,
        ...
        cl_strategy = FedProtoStrategy(
            net, proto_config, num_clients=cfg.server.num_clients
        )
        return cl_strategy, None
    elif strategy_name == "fedrcil":
        rc_config = getattr(cfg, "fedrcil", {})
        cl_strategy = FedRCILStrategy(
            net,
            rc_config,
            num_clients=cfg.server.num_clients,
            buffer_size=rc_config.get("buffer_size", 200),
        )
        return cl_strategy, None
        ...
        cl_strategy = FedWeITStrategy(
            net,
            sparsity=weit_config.get("sparsity", 0.5),
            num_clients=cfg.server.num_clients,
            l1_lambda=weit_config.get("l1_lambda", 0.1),
            l2_lambda=weit_config.get("l2_lambda", 100.0),
            device=str(DEVICE),
        )
        return cl_strategy, None
        ...
        cl_strategy = SacFLStrategy(
            net, sacfl_config, num_clients=cfg.server.num_clients
        )
        return cl_strategy, None
    elif strategy_name == "stamp":
        stamp_config = getattr(cfg, "stamp", {})
        cl_strategy = STAMPStrategy(
            net, stamp_config, num_clients=cfg.server.num_clients
        )
        return cl_strategy, None
    ...
    if strategy_name == "naive" or strategy_name == "domain":
        ...
        cl_strategy = Replay(mem_size=mem_size, **common_params)
        ...
        plugins = [EWCPlugin(ewc_lambda=ewc_lambda), ReplayPlugin(mem_size=mem_size)]
        cl_strategy = Naive(plugins=plugins, **common_params)
    else:
