...
class FedWeITStrategy:
    def __init__(
        ...
        **kwargs,
        ...
        self.client_task_masks = [
            {} for _ in range(num_clients)
        ]  # list of dicts: client -> {task_id: mask}
        self.client_task_adaptives = [
            {} for _ in range(num_clients)
        ]  # list of dicts: client -> {task_id: task-adaptive param}
        # initialize attention weights for inter-client transfer
        self.client_task_attn = [
            {} for _ in range(num_clients)
        ]  # list of dicts: client -> {task_id: attn vector}
    ...
    def _init_task_adaptive(self, param):
        ...
        return nn.Parameter(
            torch.zeros_like(param, device=self.device), requires_grad=True
        )
    ...
    def decompose_params(
        self, base_params, mask, task_adaptive, kb_attn=None, kb_params=None
    ):
    ...
    def train_task(
        self, data, task_id: int, client_id: int, epochs: int = 1, lr: float = 1e-3
    ):
        ...
        if task_id not in self.client_task_masks[client_id]:
            ...
            for name, param in self.model.named_parameters():
                self.client_task_masks[client_id][task_id][name] = self._init_mask(
                    param, self.sparsity
                )
                self.client_task_adaptives[client_id][task_id][name] = (
                    self._init_task_adaptive(param)
                )
        ...
        if len(kb_params) > 0:
            if task_id not in self.client_task_attn[client_id]:
                self.client_task_attn[client_id][task_id] = self._init_attn(
                    len(kb_params)
                )
        ...
        params = self.decompose_params(
            ...
            task_adaptive,
            attn,
            kb_params if attn is not None else None,
        )
        ...
        for epoch in range(epochs):
            ...
            for x, y in data:
                ...
                l1 = sum(torch.abs(m).sum() for m in mask.values()) + sum(
                    torch.abs(a).sum() for a in task_adaptive.values()
                )
        ...
        kb_entry = (
            client_id,
            task_id,
            {k: v.detach().clone() for k, v in task_adaptive.items()},
        )
    ...
    def get_sparse_update(self, client_id, task_id):
        ...
        task_adaptive = self.client_task_adaptives[client_id][task_id]
        masked_base = {
            k: self.base_params[k] * mask[k].detach() for k in self.base_params
        }
    ...
    def eval(self, test_stream):
        ...
        for experience in test_stream:
            ...
            if task_id not in self.client_task_masks[client_id]:
                print(
                    f"[FedWeIT] Skipping evaluation on task {task_id} - not trained yet"
                )
                continue
