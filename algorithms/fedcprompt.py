...
class FedCPromptStrategy:
    def __init__(
        ...
        **kwargs,
        ...
        self.client_classifiers = [
            self._init_classifier_pool() for _ in range(num_clients)
        ]
    ...
    def get_prompt_params(self, client_id: int, task_id: int):
        ...
        return {
            k: v.data.clone()
            for k, v in self.client_prompt_pools[client_id][task_id].items()
        }

    def set_prompt_params(
        self, prompt_params: Dict[str, Any], client_id: int, task_id: int
    ):
    ...
    def set_classifier_params(
        self, clf_params: Dict[str, Any], client_id: int, task_id: int
    ):
    ...
    def aggregate(
        self,
        client_prompt_pools: List[List[Dict[str, torch.Tensor]]],
        client_classifier_pools: List[List[nn.Module]],
    ):
        ...
        for t in range(self.num_tasks):
            ...
            for k in ["key", "value", "attn"]:
                stack = torch.stack(
                    [client_prompt_pools[c][t][k] for c in range(self.num_clients)],
                    dim=0,
                )
            ...
            state_dicts = [
                client_classifier_pools[c][t].state_dict()
    ...
    def train_round(self, data, task_id: int, client_id: int):
        ...
        return self.get_prompt_params(client_id, task_id), self.get_classifier_params(
            client_id, task_id
        )
