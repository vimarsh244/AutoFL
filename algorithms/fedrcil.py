...
class FedRCILStrategy:
    def __init__(
        ...
        **kwargs,
        ...
        self.buffer_size = buffer_size
        self.client_distills = [
            self.initialize_distillation() for _ in range(num_clients)
        ]
        ...
        self.current_task = 0
        self.task_class_map = kwargs.get(
            "task_class_map", None
        )  # dict: task_id -> class list
        # logging
    ...
    def get_distillation_params(self):
        return {
            "temperature": self.rc_config.get("temperature", 0.5),
            "projection": self.model.projection.weight.data.clone(),
        }
    ...
    def aggregate(
        self, client_models: List[nn.Module], client_distills: List[Dict[str, Any]]
    ):
