...
def test_sacfl_strategy():
    ...
    print(
        "Aggregated encoder weights (first layer):",
        list(agg_enc.parameters())[0].flatten()[:5],
    )

    # Check decoder locality
    print(
        "Client 0 decoder weights (first layer):",
        list(strategy.client_decoders[0].parameters())[0].flatten()[:5],
    )
    print(
        "Client 1 decoder weights (first layer):",
        list(strategy.client_decoders[1].parameters())[0].flatten()[:5],
    )
    ...
    assert (
        triggered0 or triggered1
    ), "At least one client should trigger shift detection in this test."
