...
class SimpleCNN(nn.Module):
    def __init__(self, num_classes=10):
        super().__init__()
        self.conv = nn.Sequential(nn.Conv2d(1, 16, 3, 1), nn.ReLU(), nn.Flatten())
...
def test_stamp_minimal():
    ...
    try:
        ...
        stamp = STAMPStrategy(
            net,
            stamp_config={"coreset_size": 8, "feature_layer": "conv", "device": "cpu"},
        ...
        for cid in range(num_clients):
            xs = torch.stack(
                [x if x.dim() == 3 else x.unsqueeze(0) for x, y in data_clients[cid]]
            )
            ys = torch.stack([y for x, y in data_clients[cid]])
            coreset, protos = stamp.train_round(
                (xs, ys), task_id=0, client_id=cid, lambda_proto=1.0
            )
            print(
                f"client {cid} coreset size: {len(coreset)}; prototypes: {list(protos.keys())}"
            )
            client_protos.append(protos)
        # aggregate
        agg_coreset, global_protos = stamp.aggregate(
            [stamp.get_coreset_params(0), stamp.get_coreset_params(1)], client_protos
        )
        print(
            f"aggregated coreset size: {len(agg_coreset)}; global prototypes: {list(global_protos.keys())}"
        )
