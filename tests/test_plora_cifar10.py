...
def test_plora_cifar10():
    ...
    try:
        ...
        print(
            f"Loaded train: {len(trainloader)} batches, val: {len(valloader)}, test: {len(testloader)}"
        )
        # Create model and strategy
        net = Net()
        plora = PLoRAStrategy(
            net, plora_config={"rank": 4, "alpha": 1.0}, num_clients=1
        )
        ...
        for epoch in range(epochs):
            ...
            for batch in trainloader:
                ...
                lora_params = [
                    p
                    for n, m in plora.model.named_modules()
                    if hasattr(m, "lora_a")
