...
def test_async_strategy_fedasync():
    ...
    try:
        ...
        print(f"✓ FedAsync aggregation works (result value: {result_arrays[0][0, 0]:.4f})")
...
def test_async_strategy_asyncfeded():
    ...
    try:
        ...
        print(f"✓ AsyncFedED aggregation works (result value: {result_arrays[0][0, 0]:.4f})")
...
def test_staleness_weights():
    ...
    try:
        ...
        print(f"✓ Polynomial decay: t=0 -> {weight_0:.4f}, t=1 -> {weight_1:.4f}, t=5 -> {weight_5:.4f}")
        
        # Test hinge function
        weight_b3 = strategy.get_staleness_weight_coeff_fedasync_hinge(t_diff=3, a=10, b=4)
        weight_b5 = strategy.get_staleness_weight_coeff_fedasync_hinge(t_diff=5, a=10, b=4)
        
        assert weight_b3 == 1.0, "t_diff < b should give weight 1.0"
        assert weight_b5 < 1.0, "t_diff > b should give weight < 1.0"
        print(f"✓ Hinge function: t=3 (b=4) -> {weight_b3:.4f}, t=5 (b=4) -> {weight_b5:.4f}")
...
def test_async_history():
    ...
    try:
        ...
        history.add_metrics_centralized_async(
            metrics={"accuracy": 0.7}, timestamp=1.0
        )
        history.add_metrics_centralized_async(
            metrics={"accuracy": 0.8}, timestamp=2.0
        )
